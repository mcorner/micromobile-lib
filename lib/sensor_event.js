import BufferedSensor from './buffered_sensor';
  ...
  start(){
    ...
    for (var i = 0; i < this.sensors.length; i++){
      ...
      promises.push(new Promise((resolve,reject) =>{
        setTimeout(function() {
          //  The totalSamples we will gather is the sample # thus far minus the starting sample #
          // plus the buffer on the start and end, determined by the ms of buffer * sensor rate
          const totalSamples = sensor.sensor.totalSamples - sensor.startSample + (2 * sensor.rate * sensor.buffer/1000.0);
